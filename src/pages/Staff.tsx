              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} viewport={{ once: true }}>
                <MilitaryCard>
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 rounded-sm bg-secondary flex items-center justify-center flex-shrink-0 border border-radar overflow-hidden">
                      {member.image ? <img src={member.image} alt={member.name} className="w-full h-full object-cover" /> : <User className="w-7 h-7 text-muted-foreground" />}
                    </div>
                    <div>
